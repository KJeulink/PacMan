...
void Pinky::moveHelper(std::vector<std::vector<int>> &map, std::vector<Direction> options) {
	int back_chance;
	if (options.size() == 0) {
		back_chance = 5;
	...
	std::srand(std::time(nullptr) + 99); // use current time as seed for random generator
