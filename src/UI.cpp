...
UI::UI(std::vector<std::vector<int>> map)
{
...
void UI::drawScore()
{
    ...
    while (digitSelector < 100000) {
        ...
        SDL_Rect dst = {100 - digit * 24, 27 * 24, 24, 24};
...
void UI::drawLives()
{
    for (int i = 0; i < lives; i++) {
        SDL_Rect dst = {27 * 24 - i * 24, 27 * 24, 24, 24};
...
void UI::init()
{
    ...
    window = SDL_CreateWindow("Pacman", SDL_WINDOWPOS_CENTERED,
                              SDL_WINDOWPOS_CENTERED, 672, 648 + (24 + 4),
                              SDL_WINDOW_SHOWN);
...
void UI::drawBackground(std::vector<std::vector<int>> &map)
{
    ...
    for (size_t i = 0; i < map.size(); i++) {
        for (size_t j = 0; j < map[i].size(); j++) {
            if (map[i][j] == 1) {
                SDL_Rect dst = {static_cast<int>(j) * 24,
                                static_cast<int>(i) * 24, 24, 24};
